...
from attention_conv import Attention, AttentionConv
...
class SiTBlockConv(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0,
        ...
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = AttentionConv(hidden_size, num_heads=num_heads, qkv_bias=True,
                              stride=stride, kernel_size=kernel_size, **block_kwargs)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        self.mlp = Mlp(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu, drop=0)
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        num_classes=1000,
        learn_sigma=True,
        stride_list = [1,1,2,2,2,4,4,2,2,2,1,1],
        kernel_size_list = [1,1,2,2,2,4,4,2,2,2,1,1]
    ):
        super().__init__()
        ...
        assert len(stride_list) == len(kernel_size_list), "stride_list and kernel_size_list must have the same length"
        assert len(stride_list) == depth
        print("Initializing LightningDiT with the following block configurations:")
        print("---------------------------------------")
        ...
        for i in range(depth):
            ...
            if stride == 1 and kernel_size == 1:
                ...
                print(f"idx={i}: add full block")
            else:
                block = SiTBlockConv(hidden_size, num_heads, mlp_ratio=mlp_ratio,
                                     stride=stride, kernel_size=kernel_size)
                print(f"idx={i}: stride={stride}, kernel_size={kernel_size}")
