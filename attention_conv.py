...
__all__ = ['Attention', 'AttentionConv', 'AttentionRope', 'maybe_add_mask', 'resolve_self_attn_mask']
...
class AttentionConv(nn.Module):
    ...
    def __init__(
        ...
        self.k_conv = nn.Conv2d(
            ...
            stride=stride, 
            groups=self.head_dim,  # 深度可分离卷积，每个通道独立卷积
            bias=False,  # 避免偏置带来的内存布局问题
        ...
        print("kernel_size:", kernel_size, f"stride:{stride}")
        # ------------------------------------------- #
    ...
    def forward(
        ...
        # --------------------------------------------------------- #
        H = W = int(N ** 0.5)  # 假设是正方形的 patch grid
        assert H * W == N, f"序列长度 {N} 必须是完全平方数"
        # 重塑 k 和 v 为 (B*num_heads, head_dim, H, W) 格式进行卷积
        k_reshaped = k.permute(0, 1, 3, 2).reshape(B*self.num_heads, self.head_dim, H, W).contiguous()
        v_reshaped = v.permute(0, 1, 3, 2).reshape(B*self.num_heads, self.head_dim, H, W).contiguous()
        k_downsampled = self.k_conv(k_reshaped).contiguous()  # (B*num_heads, head_dim, H//f, W//f)
        v_downsampled = self.v_conv(v_reshaped).contiguous()  # (B*num_heads, head_dim, H//f, W//f)
        # 重新整理为注意力计算所需的形状
        H_new, W_new = H // self.downsample_factor, W // self.downsample_factor
        N_new = H_new * W_new
        k = k_downsampled.reshape(B, self.num_heads, self.head_dim, N_new).permute(0, 1, 3, 2).contiguous()
        v = v_downsampled.reshape(B, self.num_heads, self.head_dim, N_new).permute(0, 1, 3, 2).contiguous()
        # --------------------------------------------------------- #
